...
func handle_environmental_damage(peer_id: int, chunk_pos: Vector2i, object_id: int, damage: float, hit_position: Vector3) -> void:
	...
	if was_destroyed:
		...
		if object_type == "truffula_tree":
			# Spawn a fallen log at ground level
...
func handle_dynamic_object_damage(peer_id: int, object_name: String, damage: float, hit_position: Vector3) -> void:
	...
	if obj_data["health"] <= 0:
		...
		if object_type == "fallen_log":
			...
			# When log tips over, its length axis points IN the fall direction
			var log_length_axis := Vector3(sin(fall_angle), 0, cos(fall_angle))

			# Spawn 2 split logs along the fallen log's length
			for i in 2:
			...
			for i in split_positions.size():
				...
				# Track the split log - each gives 10-15 wood
			...
			# Pass fall_angle directly - split logs should point same direction as fallen log
			NetworkManager.rpc_spawn_split_logs.rpc(split_positions, split_ids, fall_angle)
