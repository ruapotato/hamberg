...
var synced_rotation_y: float = 0.0  # Global/world rotation for network sync (absolute direction)
...
func _interpolate_remote_player(delta: float) -> void:
	...
	if body_container:
		# Smooth rotation sync - using global rotation for absolute world direction
		body_container.global_rotation.y = lerp_angle(body_container.global_rotation.y, target_rot, 0.3)
...
func _update_body_animations(delta: float) -> void:
	...
	if is_special_attacking:
		...
		# Sword jab: long forward thrust animation (Valheim-style)
		...
		# Knife lunge: forward thrust (uses same jab-like motion)
			...
			var lunge_progress = special_attack_timer / current_special_attack_animation_time
			_animate_sword_jab(lunge_progress, right_arm, _right_elbow)
